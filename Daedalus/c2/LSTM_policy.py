...
class DefenderPolicy:
    def __init__(self):
        ...
        # 1 manipulated value + 4 node states + 1 cowrie + 1 fake edge deployed + 1 fake data + 4 attacker perceived states, for each host in network
        num_hosts = len(self.network.clients)
        observation_shape = (num_hosts * 1) + (num_hosts * 4) + (num_hosts * 1) + (num_hosts * 1) + (num_hosts * 1) + (num_hosts * 4)
