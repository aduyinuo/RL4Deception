...
class HostSelectionNetwork(nn.Module):
    def __init__(self, num_hosts, input_shape):
        super(HostSelectionNetwork, self).__init__()
        self.fc1 = nn.Linear(input_shape, 64)
        self.fc2 = nn.Linear(64, 32)
        self.fc3 = nn.Linear(32, num_hosts)
...
class CombinedNetwork(nn.Module):
    def __init__(self, num_hosts, input_shape, lstm_units=32):
        ...
        self.lstm = nn.LSTM(input_shape, lstm_units, batch_first=True)
        
        # feed LSTM output into the host NN. The input shape is the number of LSTM units
        self.host_selection_network = HostSelectionNetwork(num_hosts, lstm_units)

        # the input shape of the action NN is the LSTM shape and num_hosts as we feed output of LSTM and Host into action NN
        self.action_selection_network = ActionSelectionNetwork(lstm_units + num_hosts)

    # each time step, we take input vector and state of past LSTM layer
    def forward(self, inputs):
        lstm_output, hidden_state = self.lstm(inputs)
        lstm_output = lstm_output[:, -1, :]  # Get the output of the last time step

        host_selection_output = self.host_selection_network(lstm_output)

        host_selection_sample = torch.multinomial(host_selection_output, 1).squeeze()
        host_selection_sample_one_hot = nn.functional.one_hot(host_selection_sample, num_classes=host_selection_output.size(-1)).float()
        host_selection_sample_one_hot = host_selection_sample_one_hot.unsqueeze(0)

        print(f"host selection: {host_selection_sample_one_hot}")
        # Print out the dimensions
        print(f"lstm_output shape: {lstm_output.shape}")
        print(f"host_selection_sample_one_hot shape: {host_selection_sample_one_hot.shape}")

        action_inputs = torch.cat((lstm_output, host_selection_sample_one_hot), dim=-1)
        action_selection_output = self.action_selection_network(action_inputs)

        return host_selection_output, host_selection_sample, action_selection_output, hidden_state
...
class DefenderPolicy:
    ...
    def choose_action(self, current_observation):
        ...
        embedded_vector = torch.tensor(embedded_vector, dtype=torch.float32)

        print(f"input sequence {embedded_vector}")

        # testing purposes -- checking that state is retained throughout time steps
        print(f"old state at t-1 step: {self.hidden_state}")

        # get the probabilities and output from the model
        host_probs, host_selection_sample, action_probs, new_lstm_hidden_state = self.model(embedded_vector)

        # testing purposes -- checking that state is retained throughout time steps
        self.hidden_state = new_lstm_hidden_state
        print(f"new state at t step: {self.hidden_state}")
        
        # normalize the probabilities to sum to 1 -- need to detach tensor to use numpy arrays
        action_probs = action_probs.detach().numpy().flatten()
        
        host_selection_sample = host_selection_sample.detach().numpy().flatten()
        ...
        # add current observation to sequence to be embedded for next time step
