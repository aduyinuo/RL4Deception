...
class CombinedNetwork(tf.keras.Model):
    ...
    def call(self, inputs, training=False):
        ...
        print(f"host_selection_sample_one_hot shape: {host_selection_sample.shape}")
...
class DefenderPolicy:
    ...
    def choose_action(self, current_observation):
        ...
        print(f"old state at t-1 step: {self.lstm_state}")

        # get the probabilities and output from the model
        host_probs, host_selection_sample, action_probs, new_lstm_state = self.model(embedded_vector)

        # testing purposes -- checking that state is retained throughout time steps
        self.lstm_state = new_lstm_state
        print(f"new state at t step: {self.lstm_state}")
